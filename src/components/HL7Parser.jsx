...
const HL7Parser = () => {
    ...
    const handleSend = async () => { if (!isAuthenticated) return; setIsSending(true); setResponse(null); try { const result = await sendHl7(host, port, rebuildHl7Message(segments)); setResponse({ success: true, ...result }); } catch (e) { setResponse({ success: false, message: e.message }); } finally { setIsSending(false); } };
                    ...
                    <div className="flex flex-col md:flex-row gap-8">
                        ...
                        <div className={`w-full ${analysisResult || isAnalyzing ? 'md:w-2/4 lg:w-2/5' : 'md:w-3/4 lg:w-4/5'} transition-all duration-300`}>
                            <div className="flex flex-col gap-8">
                                <div className="sticky top-4 z-10 bg-gray-900 py-2">
